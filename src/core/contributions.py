...
def extract_signature_details(commit_sha, repo_path):
    ...
    try:
        ...
        signature_match = re.search(r'gpg: Signature made (.*)\ngpg:.*using (.*) key ([\w]+)', output)
        if signature_match:
...
def analyze_commits(repo, repo_path):
    ...
    for commit in repo.iter_commits():
        ...
        if key_id:
            ...
            if commit.parents:
                diff = repo.git.diff(commit.parents[0], commit, '--numstat', '--no-renames').strip()
                ...
                diff = repo.git.diff_tree('--numstat', '--no-renames', '--root', commit.hexsha).strip()
                for line in diff.split('\n'):
                    if line:
                        parts = line.split()
                        if len(parts) >= 2 and parts[0].isdigit():
    ...
    for identity in identities.values():
        identity['contribution_percentage'] = (identity['contribution_percentage'] / total_loc) * 100 if total_loc > 0 else 0
...
def get_final_contributors_list(contributions):
    ...
    for identity in contributions['identities'].values():
        ...
        aggregated_contributors[key]['contribution_percentage'] += identity['contribution_percentage']
